...
audio.addEventListener('timeupdate', () => {
    // Отправляем статус раз в несколько секунд на всякий случай
    if (Math.floor(audio.currentTime) % 5 === 0) syncState();
});       
...
function syncState() {
    ...
    window.parent.postMessage({
        type: 'PLAYER_STATE',
        trackSrc: songs[songIndex].src, // Просто .src без getAttribute
