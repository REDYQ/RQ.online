...
function updateTrackListVisuals() {
    ...
    allTracks.forEach(track => {
        ...
        // Находим блок с полосочками внутри конкретного трека
        const wave = track.querySelector('.playing-bars');

        // Проверяем, совпадает ли этот трек с тем, что сейчас в плеере
        const isThisTrack = trackSrc && currentPlayingSrc && 
                           (trackSrc.includes(currentPlayingSrc) || currentPlayingSrc.includes(trackSrc));

        if (isThisTrack) {
            // 1. Всегда добавляем active, если это тот самый трек (даже на паузе)
            track.classList.add('active'); 
            
            // 2. Управляем классом playing и отображением полосочек
            if (isCurrentlyPlaying) {
                track.classList.add('playing');
                if (wave) wave.style.display = 'flex'; // Включаем анимацию
            } else {
                track.classList.remove('playing');
                if (wave) wave.style.display = 'none'; // Выключаем на паузе
            }
        } else {
            // 3. Если трек другой или мы сменили папку — убираем всё
