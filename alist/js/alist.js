...
async function renderList(data, containerId, isMyList = false, userItems = []) {
    ...
    if (isMyList && data.length > 0) {
        ...
        const countText = myListMode === 'titles' 
            ? `${myItems.length} ${getNoun(myItems.length, 'тайтл', 'тайтла', 'тайтлов')}` 
            : `${data.length} ${getNoun(data.length, 'папка', 'папки', 'папок')}`;
...
function handleMusicClick(event, hasMusic, name) {
    ...
    if (hasMusic) {
        window.open(`https://redyq.github.io/RQ.online/amusic`, '_blank');
    } else {
...
function handleMusicByTestClick(event, hasMusicByTest, name) {
    ...
    if (hasMusicByTest) {
        window.open(`https://redyq.github.io/RQ.online/test/amusic`, '_blank');
    } else {
